...
resource "aws_launch_template" "gitlab_runner_launch_template" {
  ...
  user_data                     = base64encode(templatefile("${path.module}/scripts/gitlab_runner.sh", {
    RUNNER_TOKEN_01             = var.runner_token_01
    RUNNER_TOKEN_02             = var.runner_token_02
    RUNNER_TOKEN_03             = var.runner_token_03
    ACCOUNT_ID                  = var.account_id
    REGION                      = var.region
    TERRAFORM_VERSION           = var.tf_version
    TERRAGRUNT_VERSION          = var.tg_version
    MAVEN_VERSION               = var.mvn_version
